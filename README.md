...
> **Note:** This is a fork of the [legid](https://github.com/shuding/legid) package. The original `legid` package uses `crypto.subtle.digest` which requires async functions. This fork uses the `js-sha1` package to make all functions **synchronous**, eliminating the need for `async`/`await` when generating and verifying IDs.
...
```typescript
import { createId } from 'legid-sync'
...
```typescript
import { verifyId } from 'legid-sync'
...
```typescript
import { createId, verifyId } from 'legid-sync'
